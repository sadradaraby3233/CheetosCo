...
const MenuSystem = (() => {
  ...
  function back() {
    ...
    if (!menu) return;
    if (stack.length === 1) {
      // Root menu: call onBack to let the game decide
      SoundBank.menuBack();
      stack.pop();
  ...
  function handleKey(e) {
    ...
    switch (e.key) {
      case 'ArrowUp': e.preventDefault(); navigateUp(); return true;
      case 'ArrowDown': e.preventDefault(); navigateDown(); return true;
      case 'Home': e.preventDefault(); goToTop(); return true;
      case 'End': e.preventDefault(); goToBottom(); return true;
      case 'ArrowLeft': e.preventDefault(); navigateLeft(); return true;
      case 'ArrowRight': e.preventDefault(); navigateRight(); return true;
      case 'Enter': e.preventDefault(); select(); return true;
      case 'Escape': e.preventDefault(); back(); return true;
      case ' ': e.preventDefault(); repeat(); return true;
  ...
  document.addEventListener('touchend', (e) => {
    if (isTwoFingerActive) {
      ...
      if (e.touches.length < 2) isTwoFingerActive = false;
    }
  }, { passive: false });

  return {
    open, close, select, back, repeat,
    navigateUp, navigateDown, goToTop, goToBottom,
    announce, replaceItems, isOpen, current, handleKey
  };
})();
